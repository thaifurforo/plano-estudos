# 📚 Plano de Estudos - Preparação para Entrevistas Tech

Bem-vindo ao seu guia completo de preparação para entrevistas técnicas! Este site contém planos de estudos estruturados e organizados para ajudá-lo a conquistar sua vaga em empresas de tecnologia.

## 🎯 Visão Geral

Este repositório oferece **três pilares fundamentais** para sua preparação:

### 💻 LeetCode
Domine os **150 problemas mais importantes** do LeetCode para entrevistas técnicas em empresas FAANG e similares.

[➡️ Começar com LeetCode](leetcode/index.md)

### 🏗️ System Design
Aprenda a projetar sistemas escaláveis e robustos com **10+ estudos de caso** de sistemas reais.

[➡️ Explorar System Design](system-design/index.md)

### 🤝 Soft Skills
Desenvolva habilidades comportamentais essenciais com o **método STAR** e 20+ exemplos práticos.

[➡️ Desenvolver Soft Skills](soft-skills/index.md)

## 📊 Estatísticas

- **150** problemas LeetCode prioritários
- **10+** estudos de caso de System Design
- **20+** exemplos de entrevistas comportamentais
- **26 semanas** de conteúdo (~6 meses)

## 🗓️ Cronograma Geral

| Período | Foco | Duração |
|---------|------|---------|
| **Semanas 1-12** | LeetCode | 12 semanas |
| **Semanas 13-20** | System Design | 8 semanas |
| **Semanas 21-26** | Soft Skills | 6 semanas |

## 🚀 Como Começar

1. **Escolha seu foco**: Comece pela área que você mais precisa melhorar
2. **Siga o cronograma**: Cada plano tem um cronograma detalhado
3. **Pratique consistentemente**: 2-3 horas por dia é o ideal
4. **Acompanhe seu progresso**: Use a planilha de tracking
5. **Revise regularmente**: Revisão espaçada é fundamental

## 📈 Acompanhamento

Utilize a planilha Google Sheets para acompanhar seu progresso:

👉 **[INSERIR_LINK_DA_PLANILHA_AQUI]**

## 🎓 Recursos Adicionais

- [LeetCode](https://leetcode.com/) - Plataforma principal de problemas
- [NeetCode](https://neetcode.io/) - Vídeos explicativos
- [System Design Primer](https://github.com/donnemartin/system-design-primer) - Guia completo
- [Cracking the Coding Interview](https://www.crackingthecodinginterview.com/) - Livro essencial

---

**Boa sorte nos estudos! 🚀**